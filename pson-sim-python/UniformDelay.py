...
for j in range(0,len(test_iter_list)):  
    for i in range(0,len(test_load_list)):
        ...
        filename="d:/2017/rhapsody/PSONScheduler/component_4/DefaultConfig/sim_result_D100000_S"
    ...
    #for i in range(0,5):
    plt.semilogy(test_load_list,average_delay[j][:])
...
basic = plt.hist(basicC, 50, normed=1, histtype='step', cumulative=False, log=True )

ga = plt.hist(gaC, 1000, normed=1, histtype='step', cumulative=-1, color = 'g', log=True )
gapa = plt.hist(gapaC, 1000, normed=1, histtype='step', cumulative=-1, color = 'b' , alpha = 0.5, log=True)
